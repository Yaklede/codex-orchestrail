...
Status is compact by default. For an untracked session, report that no durable run exists; do not start one. Request `{"detail":true}` only for specific history or output questions. `revision` counts all log events; `controlRevision` tracks task changes. Neither measures tokens.
